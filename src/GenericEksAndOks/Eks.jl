...
Insert the Jordan-Wigner strings needed to represent a parity-even product of
fermionic operators as a product of local operators.

Operators are first stably sorted by site. The sign generated by permuting the
fermionic operators is included in the result, and the product is then swept
from right to left to insert `F` wherever the fermion parity to the right is
odd. Parity-odd products are not supported since their Jordan-Wigner string
depends on a boundary convention.
"""
function insert_JW_string(p::ITensors.Prod{ITensors.Op}, hilbert::Array)
    # `collect(p)` currently returns `Vector{Any}`. Preserve the element type so
    # constructing a new Prod dispatches to the Op-specific algebra below.
    ops = ITensors.Op[op for op in p]
    isfermionic(op) = ITensors.has_fermion_string(
        ITensors.which_op(op), hilbert[only(ITensors.sites(op))]
    )

    any(isfermionic, ops) || return p

    # Sorting operators on distinct sites is harmless for local operators, but
    # every exchange of two fermionic operators contributes a minus sign.
    perm = sortperm(
        ops;
        alg=Base.Sort.InsertionSort,
        by=op -> only(ITensors.sites(op)),
    )
    ops = ops[perm]
    fermionic = isfermionic.(ops)
    fermionic_perm = perm[fermionic]
    permutation_sign = ITensors.parity_sign(fermionic_perm)

    jw_ops = ITensors.Op[]
    previous_site = length(hilbert) + 1
    odd_parity_to_right = false

    for n in reverse(eachindex(ops))
        site_n = only(ITensors.sites(ops[n]))

        # This is the rightmost operator on a new site. If the operators to its
        # right have odd parity, terminate their JW string on this site and fill
        # any unoccupied sites in the gap.
        if site_n < previous_site
            if odd_parity_to_right
                for site in (site_n + 1):(previous_site - 1)
                    push!(jw_ops, ITensors.Op("F", site))
                end

                op_n = ops[n]
                ops[n] = ITensors.Op(
                    "$(ITensors.which_op(op_n)) * F",
                    site_n;
                    ITensors.params(op_n)...,
                )
            end
            previous_site = site_n
        end

        odd_parity_to_right = xor(odd_parity_to_right, fermionic[n])
    end

    odd_parity_to_right && error(
        "Parity-odd fermionic operator products are not supported by insert_JW_string"
    )

    return permutation_sign * ITensors.Prod(vcat(ops, jw_ops))
end
...
function TensorOperatorSum(ham::OpSum, hilbert::Array; combine_tensors=true, sort_diag=true)
    ...
    for (i, o) in enumerate(ham)
        # `insert_JW_string` is a no-op for bosonic terms. Calling it
        # unconditionally also handles Electron, tJ, and custom site types that
        # define fermionic operators without carrying the literal "Fermion" tag.
        o = insert_JW_string(o, hilbert_flat)
